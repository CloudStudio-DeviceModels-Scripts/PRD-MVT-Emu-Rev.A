...
//   endpoints.addEndpoint("1", "", endpointType.temperatureSensor);
  // endpoints.addEndpoint("2", "CO2 sensor", endpointType.ppmConcentrationSensor, ppmConcentrationSensorSubType.carbonDioxide);

    var statusCPUTempAvg = endpoints.addEndpoint("statusCPUTempAvg", "CPU Temp Avg", endpointType.temperatureSensor);
    
    var statusDevTtyUSB0 = endpoints.addEndpoint("statusDevTtyUSB0", "DevTtyUSB0", endpointType.genericSensor);
    statusDevTtyUSB0.variableTypeId = 1050;

    var mqttConnected = endpoints.addEndpoint("mqttConnected", "MQTT Connected", endpointType.genericSensor);
    mqttConnected.variableTypeId = 1050;

    var startTime = endpoints.addEndpoint("startTime", "App Start Time", endpointType.genericSensor);
    startTime.variableTypeId = 1051;

    var version = endpoints.addEndpoint("version", "Version 1.1.X", endpointType.genericSensor);
    version.variableTypeId = 1052;

    var alert = statusCPUTempAvg.addAlert();
